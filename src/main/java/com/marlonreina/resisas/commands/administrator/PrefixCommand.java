...
public class PrefixCommand implements Command {
    ...
    public void execute(MessageReceivedEvent event, String[] args) {
        ...
        if (!Objects.requireNonNull(event.getMember()).hasPermission(Permission.ADMINISTRATOR)) {
            event.getChannel().sendMessageEmbeds(EmbedUtil.error("No tienes permisos para ejecutar este comando.").build()).queue();
            return;
        }

        var currentPrefix = guildService.getOrCreate(event.getGuild().getId().toString()).getPrefix();

        if (args.length < 1) {
            event.getChannel().sendMessageEmbeds(EmbedUtil.simplyBuildMessage(
        ...
        event.getChannel().sendMessageEmbeds(EmbedUtil.success("Prefijo actualizado a: " + newPrefix).build()).queue();
    }
}